...
int addCell(PlacementDB& db, const std::string& n, double x, double y, double w, double h, bool term=false) {
    int id = db.addCell(n, w, h, term); db.setCellLocation(n, x, y, term); return id;
}
...
int main() {
    ...
    { PlacementDB db; addCoreRow(db,0,0,20,20); int f=addCell(db,"f",0,0,10,10,true); BinGrid g(db,1,1,0.5); (void)f; expectNear(g.bin(0).fixed_area,100); expectNear(g.bin(0).movable_area,0); expectNear(g.bin(0).movable_capacity,100); }
    ...
    { PlacementDB db; addCoreRow(db,0,0,20,20); addCell(db,"m1",0,0,10,10); addCell(db,"m2",10,10,20,20); addCell(db,"f",0,10,10,10,true); BinGrid g(db,2,2,1.0); expectNear(g.totalMovableArea(),200); expectNear(g.totalFixedArea(),100); }
    return 0;
}
