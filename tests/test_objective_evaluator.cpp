...
int main(){
 {PlacementDB db; row(db); int a=cell(db,"a",0,0,1,1), b=cell(db,"b",10,0,1,1); pin2(db,a,b); BinGrid g(db,1,1,1); auto m=ObjectiveEvaluator{}.evaluate(db,g,2); near(m.hpwl,HPWLEvaluator{}.totalHPWL(db)); near(m.density_penalty,0); near(m.weighted_density_penalty,0); near(m.total_cost,m.normalized_hpwl);} 
 {PlacementDB db; row(db,10,10); cell(db,"a",0,0,8,10); BinGrid g(db,1,1,0.5); auto m=ObjectiveEvaluator{}.evaluate(db,g,3); assert(m.density_penalty>0); assert(m.total_overflow>0); near(m.total_cost,m.normalized_hpwl+3*m.normalized_density_penalty);} 
 {PlacementDB db; row(db,10,10); cell(db,"a",0,0,8,10); BinGrid g(db,1,1,0.5); auto m=ObjectiveEvaluator{}.evaluate(db,g,0); near(m.weighted_density_penalty,0); near(m.total_cost,m.normalized_hpwl);} 
